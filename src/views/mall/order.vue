  ...
  <div class="app-container">
    ...
    >
      <el-table-column type="expand">
        <template slot-scope="props">
          ...
          >
            ...
            <div class="name">
              {{
                $t("mall_order.text.expand_goods_name", {
            ...
            <div class="price">
              {{
                $t("mall_order.text.expand_subtotal_price", {
      ...
      <el-table-column
        align="center"
        min-width="120"
        :label="$t('mall_order.table.order_sn')"
        prop="orderSn"
      />

      <el-table-column
        align="center"
        :label="$t('mall_order.table.avatar')"
        width="80"
      >
      ...
      <el-table-column
        align="center"
        :label="$t('mall_order.table.user_name')"
        prop="userName"
      />

      <el-table-column
        align="center"
        :label="$t('mall_order.table.add_time')"
        prop="addTime"
        min-width="100"
      >
        <template slot-scope="scope">
      ...
      <el-table-column
        align="center"
        :label="$t('mall_order.table.order_status')"
        prop="orderStatus"
      >
        <template slot-scope="scope">
      ...
      <el-table-column
        align="center"
        :label="$t('mall_order.table.order_price')"
        prop="orderPrice"
      >
      ...
      <el-table-column
        align="center"
        :label="$t('mall_order.table.actual_price')"
        prop="actualPrice"
      >
        <template slot-scope="scope"> {{ scope.row.actualPrice }} 元 </template>
      </el-table-column>

      <el-table-column
        align="center"
        :label="$t('mall_order.table.pay_time')"
        prop="payTime"
      />

      <el-table-column
        align="center"
        :label="$t('mall_order.table.consignee')"
        prop="consignee"
      >
        <template slot-scope="scope">
      ...
      <el-table-column
        align="center"
        :label="$t('mall_order.table.mobile')"
        prop="mobile"
        min-width="100"
      />
      ...
      <el-table-column
        align="center"
        :label="$t('mall_order.table.ship_sn')"
        prop="shipSn"
      />

      <el-table-column
        align="center"
        :label="$t('mall_order.table.ship_channel')"
        prop="shipChannel"
      />

      <el-table-column
        align="center"
        :label="$t('mall_order.table.actions')"
        width="250"
        class-name="oper"
      >
        <template slot-scope="scope">
          ...
          <el-button type="warning" size="mini" @click="handlePay(scope.row)">{{
            $t("mall_order.button.pay")
          }}</el-button>
          <el-button
            type="primary"
            size="mini"
            @click="handleShip(scope.row)"
          >{{ $t("mall_order.button.ship") }}</el-button>
          <el-button
            type="danger"
            size="mini"
            @click="handleRefund(scope.row)"
          >{{ $t("mall_order.button.refund") }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      ...
      :title="$t('mall_order.dialog.detail')"
      width="800"
    >
      <section ref="print">
        <el-form :data="orderDetail" label-position="left">
          ...
          <el-form-item :label="$t('mall_order.form.detail_receiving_info')">
            <span>{{
              $t("mall_order.text.detail_consigne", {
                consignee: orderDetail.order.consignee,
              })
            }}</span>
            <span>{{
              $t("mall_order.text.detail_mobile", {
                mobile: orderDetail.order.mobile,
              })
            }}</span>
            <span>{{
              $t("mall_order.text.detail_address", {
          ...
          <el-form-item :label="$t('mall_order.form.detail_pay_info')">
            <span>{{
              $t("mall_order.text.detail_pay_channel", {
                pay_channel: "微信支付",
              })
            }}</span>
            <span>{{
              $t("mall_order.text.detail_pay_time", {
                pay_time: orderDetail.order.payTime,
              })
            }}</span>
          </el-form-item>
          <el-form-item :label="$t('mall_order.form.detail_ship_info')">
            <span>{{
              $t("mall_order.text.detail_ship_channel", {
                ship_channel: orderDetail.order.shipChannel,
              })
            }}</span>
            <span>{{
              $t("mall_order.text.detail_ship_sn", {
                ship_sn: orderDetail.order.shipSn,
              })
            }}</span>
            <span>{{
              $t("mall_order.text.detail_ship_time", {
                ship_time: orderDetail.order.shipTime,
              })
            }}</span>
          </el-form-item>
          <el-form-item :label="$t('mall_order.form.detail_refund_info')">
            <span>{{
              $t("mall_order.text.detail_refund_amount", {
                refund_amount: orderDetail.order.refundAmount,
              })
            }}</span>
            <span>{{
              $t("mall_order.text.detail_refund_type", {
                refund_type: orderDetail.order.refundType,
              })
            }}</span>
            <span>{{
              $t("mall_order.text.detail_refund_content", {
                refund_content: orderDetail.order.refundContent,
              })
            }}</span>
            <span>{{
              $t("mall_order.text.detail_refund_time", {
                refund_time: orderDetail.order.refundTime,
              })
            }}</span>
          </el-form-item>
          <el-form-item :label="$t('mall_order.form.detail_receipt_info')">
            <span>{{
              $t("mall_order.text.detail_confirm_time", {
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="orderDialogVisible = false">{{
          $t("mall_order.button.detail_cancel")
        }}</el-button>
        <el-button type="primary" @click="printOrder">{{
          $t("mall_order.button.detail_print")
        }}</el-button>
      </span>
    </el-dialog>
    ...
    >
      ...
      >
        ...
        <el-form-item
          :label="$t('mall_order.form.pay_old_money')"
          prop="oldMoney"
        >
          <el-input-number
        ...
        <el-form-item
          :label="$t('mall_order.form.pay_new_money')"
          prop="newMoney"
        >
      ...
      <el-table :data="payForm.goodsList">
        <el-table-column
          property="goodsName"
          :label="$t('mall_order.table.pay_goods_name')"
        />
        <el-table-column
          :label="$t('mall_order.table.pay_goods_specifications')"
        >
          <template slot-scope="scope">
        ...
        <el-table-column
          property="onumber"
          width="100"
          :label="$t('mall_order.table.pay_goods_number')"
        />
    ...
    <el-dialog
      ...
      :title="$t('mall_order.dialog.ship')"
    >
      <el-form
        ref="shipForm"
        :model="shipForm"
      ...
      >
        <el-form-item
          :label="$t('mall_order.form.ship_channel')"
          prop="shipChannel"
        >
          <el-select
            v-model="shipForm.shipChannel"
            :placeholder="$t('mall_order.placeholder.ship_channel')"
          >
            <el-option
    ...
    <el-dialog
      ...
      :title="$t('mall_order.dialog.refund')"
    >
      <el-form
        ref="refundForm"
        :model="refundForm"
      ...
      >
        <el-form-item
          :label="$t('mall_order.form.refund_money')"
          prop="refundMoney"
        >
