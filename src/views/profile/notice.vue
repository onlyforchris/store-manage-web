  ...
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.title" clearable class="filter-item" style="width: 200px;"
        :placeholder="$t('profile_notice.placeholder.filter_title')" />
      <el-button class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">{{
        $t('app.button.search') }}</el-button>
    </div>

    <div class="operator-container">
      <el-button class="filter-item" type="primary" icon="el-icon-edit" @click="handleBatchRead">{{
        $t('profile_notice.button.batch_read') }}</el-button>
    ...
    <!-- 查询结果 -->
    <el-table v-loading="listLoading" :data="list" :element-loading-text="$t('app.message.list_loading')" fit
      ...
      <el-table-column align="center" :label="$t('profile_notice.table.read_time')" prop="readTime" width="120">
        <template slot-scope="scope">
          <el-tag :type="scope.row.readTime ? 'success' : 'error'">{{ $t(scope.row.readTime ?
        'profile_notice.text.read' : 'profile_notice.text.unread') }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('profile_notice.table.actions')" width="200"
        class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleRead(scope.row)">{{ $t('profile_notice.button.read')
            }}</el-button>
          <el-button type="danger" size="mini" @click="handleDelete(scope.row)">{{ $t('app.button.delete')
            }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="notice.title" :visible.sync="noticeVisible" center>
      <el-divider content-position="left">{{ $t('profile_notice.text.admin_time', {
        admin: notice.admin, time:
          notice.time
      }) }}</el-divider>
